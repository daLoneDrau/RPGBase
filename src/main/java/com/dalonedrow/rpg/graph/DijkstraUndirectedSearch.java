 ...
 *
 */
public final class DijkstraUndirectedSearch {
	 ...
	 * <tt>V</tt> - 1
	 */
    public DijkstraUndirectedSearch(final EdgeWeightedUndirectedGraph graph,
    		final int source) throws RPGException {
    	for (int i = graph.getNumberOfEdges() - 1; i >= 0; i--) {
    		if (graph.getEdge(i).getCost() < 0) {
    			throw new RPGException(ErrorMessage.INTERNAL_BAD_ARGUMENT, 
    					"edge " + graph.getEdge(i) + " has negative weight");
    		}
    	}
	...
	// e.weight()
	private boolean check(final EdgeWeightedUndirectedGraph graph, 
			final int s) {
	...
	public WeightedGraphEdge[] pathTo(final int v) {
		...
		if (hasPathTo(v)) {
			...
			for (; e != null;) {
				...
				if (e.getFrom() == lastVertex) {
					lastVertex = e.getTo();
					e = edgeTo[e.getTo()];
				} else {
					lastVertex = e.getFrom();
					e = edgeTo[e.getFrom()];
				}
			}
		}
